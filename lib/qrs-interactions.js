...
function getChildApps(blueprintId) {
	var hasBlueprint = '@' + config.qmc.childAppProp + '+so+-' + '+and+published+eq+true';
	var obj = {
		key: 'filter', 
		value: blueprintId ? '@' + config.qmc.childAppProp + '+eq+' + blueprintId + '+and+published+eq+true': hasBlueprint
	};
	return qrs.get('app/full', [obj], null);
}
...
function getCustomProperty(name) {
	return qrs.get('/qrs/CustomPropertyDefinition/')
};
