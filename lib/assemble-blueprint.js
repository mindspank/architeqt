...
function syncStory(app, def, list) {
	extend(def.qProperty, blueprintTag)
	if (list.indexOf(def.qProperty.qInfo.qId) === -1) {
		return app.createObject(def.qProperty).then(function(handle) {
...
function embedSnapshot(app, handle) {
	...
	.then(function(list) {
		return Promise.each(list.filter(function(d) { return d.qId }), function(d) {
	...
	.then(function(itemlist) {
		...
		.then(function(layouts) {
			return Promise.all(layouts.map(function(layout, index) {
				if(layout.style.id) {
